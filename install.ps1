...
function Find-WoWAddOnsPath {
    # 1. Try the Windows Registry — Blizzard writes the install root under both
    #    the 32-bit node (WOW6432Node) and, on some systems, the current-user hive.
    $RegistryKeys = @(
    ...
    foreach ($RegKey in $RegistryKeys) {
        try {
            if (Test-Path $RegKey) {
                $GamePath = (Get-ItemProperty -LiteralPath $RegKey -ErrorAction Stop).GamePath
                if ($GamePath) {
                    # GamePath may already end with _retail_; normalise either way.
                    if ($GamePath -notmatch '[/\\]_retail_[/\\]?$') {
                        $GamePath = Join-Path $GamePath '_retail_'
                    }
                    $Candidate = Join-Path $GamePath 'Interface\AddOns'
    ...
    # 3. Could not auto-detect — return $null so the caller can prompt.
    return $null
}
