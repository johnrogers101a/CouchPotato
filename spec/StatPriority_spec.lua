...
describe("StatPriority", function()
    ...
    describe("pin and lock behavior", function()
        ...
        it("OnDragStop saves position with relativePoint key", function()
            StatPriorityDB.pinned = false
            local onDragStop = ns.headerFrame:GetScript("OnDragStop")
            onDragStop()
            assert.is_not_nil(StatPriorityDB.position)
            assert.is_not_nil(StatPriorityDB.position.relativePoint)
        end)
    ...
    describe("icon sizing", function()
        ...
        it("collapse button is at least 36 wide", function()
            assert.is_not_nil(ns.collapseBtn)
            local w, _ = ns.collapseBtn:GetSize()
            assert.is_true(w >= 36, "collapse button width should be >= 36, got " .. tostring(w))
        end)
    end)

    -- =========================================================================
    -- Test 18: Dynamic re-anchoring
    -- =========================================================================
    describe("Dynamic re-anchoring", function()
        local _origTimerAfter
        local _timerCallCount

        before_each(function()
            -- Reset C_Timer state and the _cpDisabled flag before each test
            C_Timer._Reset()
            ns._cpDisabled = nil

            -- Wrap C_Timer.After to count how many times it is called
            _timerCallCount = 0
            _origTimerAfter = C_Timer.After
            C_Timer.After = function(delay, callback)
                _timerCallCount = _timerCallCount + 1
                return _origTimerAfter(delay, callback)
            end
        end)

        after_each(function()
            -- Restore original C_Timer.After
            if _origTimerAfter then
                C_Timer.After = _origTimerAfter
                _origTimerAfter = nil
            end
            ns._cpDisabled = nil
        end)

        it("deduplicates rapid QUEST_LOG_UPDATE events into a single timer", function()
            -- Fire the re-anchor event 10 times in a row.
            -- The _spReanchorPending guard means only the first event schedules
            -- a C_Timer.After call; the remaining 9 are suppressed.
            assert.is_not_nil(ns.anchorEventFrame)
            local onEvent = ns.anchorEventFrame:GetScript("OnEvent")
            assert.is_not_nil(onEvent)

            -- Fire 10 rapid events
            for _ = 1, 10 do
                onEvent(ns.anchorEventFrame, "QUEST_LOG_UPDATE")
            end

            -- Only 1 timer should have been scheduled
            assert.equals(1, _timerCallCount,
                "Expected exactly 1 C_Timer.After call after 10 rapid QUEST_LOG_UPDATE events, got " .. _timerCallCount)
        end)

        it("schedules a timer on ZONE_CHANGED_NEW_AREA", function()
            local onEvent = ns.anchorEventFrame:GetScript("OnEvent")
            onEvent(ns.anchorEventFrame, "ZONE_CHANGED_NEW_AREA")

            assert.is_true(_timerCallCount >= 1,
                "Expected at least 1 C_Timer.After call after ZONE_CHANGED_NEW_AREA")
        end)

        it("skips re-anchor scheduling when ns._cpDisabled is true", function()
            ns._cpDisabled = true

            local onEvent = ns.anchorEventFrame:GetScript("OnEvent")
            onEvent(ns.anchorEventFrame, "QUEST_WATCH_LIST_CHANGED")

            assert.equals(0, _timerCallCount,
                "Expected 0 C_Timer.After calls when _cpDisabled is true, got " .. _timerCallCount)
        end)

        it("fires the deferred re-anchor after C_Timer callback executes", function()
            -- Confirm that after the timer fires, ApplyPinnedState is invoked
            -- (detectable because the frame gets re-pointed to UIParent TOPRIGHT
            -- when no tracker is present — matching the existing ApplyPinnedState tests).
            _G.ObjectiveTrackerFrame = nil

            local onEvent = ns.anchorEventFrame:GetScript("OnEvent")
            onEvent(ns.anchorEventFrame, "PLAYER_ENTERING_WORLD")

            -- Simulate the timer callback firing
            C_Timer._FireAll()

            -- Frame should have been re-anchored; check for TOPRIGHT fallback
            local found = false
            for _, p in ipairs(ns.frame._points) do
                if p[1] == "TOPRIGHT" then found = true; break end
            end
            assert.is_true(found,
                "Expected TOPRIGHT anchor after deferred re-anchor fires with no tracker")
        end)
    end)

    -- =========================================================================
    -- Test 19: Header styling
    -- =========================================================================
    describe("Header styling", function()
        it("has a gold top-line texture on the header", function()
            -- The header frame's _textures list should include a 1px gold line.
            -- Gold color: SetColorTexture(0.9, 0.75, 0.1, 0.8)
            local header = ns.headerFrame
            assert.is_not_nil(header)
            local found = false
            for _, tex in ipairs(header._textures) do
                local c = tex._color
                if c and c[1] and c[1] > 0.8 and c[2] and c[2] > 0.6 and c[3] and c[3] < 0.3 then
                    found = true; break
                end
            end
            assert.is_true(found, "Expected at least one gold-colored texture on the header frame")
        end)

        it("has at least two gold line textures on the header (top and bottom)", function()
            local header = ns.headerFrame
            local gold_count = 0
            for _, tex in ipairs(header._textures) do
                local c = tex._color
                -- Gold: r > 0.8, g > 0.6, b < 0.3
                if c and c[1] and c[1] > 0.8 and c[2] and c[2] > 0.6 and c[3] and c[3] < 0.3 then
                    gold_count = gold_count + 1
                end
            end
            assert.is_true(gold_count >= 2,
                "Expected >= 2 gold line textures on header (top + bottom), found " .. gold_count)
        end)

        it("collapse button is exactly 36x36", function()
            assert.is_not_nil(ns.collapseBtn)
            local w, h = ns.collapseBtn:GetSize()
            assert.equals(36, w, "collapse button width should be 36, got " .. tostring(w))
            assert.equals(36, h, "collapse button height should be 36, got " .. tostring(h))
        end)
    end)

    -- =========================================================================
    -- Test 20: GetStatValue edge cases
    -- =========================================================================
    describe("GetStatValue edge cases", function()
        it("returns '?' for unrecognized stat abbreviation", function()
            -- Already have a basic "Unknown" test; this confirms any non-canonical name.
            local fn = _G.StatPriorityGetStatValue
            assert.equals("?", fn("UnknownStat"))
        end)

        it("handles nil stat abbreviation without error and returns '?'", function()
            local fn = _G.StatPriorityGetStatValue
            -- Should not throw; should return "?"
            local ok, result = pcall(fn, nil)
            -- Either it returns "?" directly or pcall succeeds with "?"
            if ok then
                assert.equals("?", result)
            else
                -- If it threw, that is a failure — the function should handle nil gracefully
                assert.is_true(false, "GetStatValue(nil) threw an error: " .. tostring(result))
            end
        end)

        it("returns '?' when all stat APIs are unavailable (UnitStat, GetHaste, etc.)", function()
            local oldUnitStat = _G.UnitStat
            local oldHaste    = _G.GetHaste
            local oldCrit     = _G.GetCritChance
            local oldMast     = _G.GetMasteryEffect
            local oldVers     = _G.GetCombatRatingBonus

            _G.UnitStat            = nil
            _G.GetHaste            = nil
            _G.GetCritChance       = nil
            _G.GetMasteryEffect    = nil
            _G.GetCombatRatingBonus = nil

            local fn = _G.StatPriorityGetStatValue
            for _, stat in ipairs({ "Int", "Str", "Agil", "Haste", "Crit", "Mast", "Vers" }) do
                assert.equals("?", fn(stat), "Expected '?' for " .. stat .. " when APIs unavailable")
            end

            _G.UnitStat             = oldUnitStat
            _G.GetHaste             = oldHaste
            _G.GetCritChance        = oldCrit
            _G.GetMasteryEffect     = oldMast
            _G.GetCombatRatingBonus = oldVers
        end)
    end)

    -- =========================================================================
    -- Test 21: commaFormat (tested indirectly via StatPriorityGetStatValue)
    -- =========================================================================
    describe("commaFormat (via GetStatValue primary-stat path)", function()
        it("formats 1,234 correctly via Int mock", function()
            _G._mockUnitStats[4] = 1234
            assert.equals("1,234", _G.StatPriorityGetStatValue("Int"))
        end)

        it("formats 1,234,567 correctly via Int mock", function()
            _G._mockUnitStats[4] = 1234567
            assert.equals("1,234,567", _G.StatPriorityGetStatValue("Int"))
        end)

        it("formats numbers under 1000 without commas", function()
            _G._mockUnitStats[4] = 999
            assert.equals("999", _G.StatPriorityGetStatValue("Int"))
        end)

        it("formats zero as '0'", function()
            -- In Lua, 0 is truthy (unlike some languages), so commaFormat(0) = "0"
            _G._mockUnitStats[4] = 0
            local val = _G.StatPriorityGetStatValue("Int")
            assert.equals("0", val,
                "GetStatValue should return '0' for zero stat value (0 is truthy in Lua)")
        end)

        it("rounds fractional values before formatting", function()
            -- commaFormat rounds to nearest integer: 1234.6 -> 1235
            _G._mockUnitStats[4] = 1234.6
            assert.equals("1,235", _G.StatPriorityGetStatValue("Int"))
        end)
    end)

    -- =========================================================================
    -- Test 22: Pin state consistency
    -- =========================================================================
    describe("Pin state consistency", function()
        it("pin button OnClick calls ApplyPinnedState when transitioning unpinned->pinned", function()
            -- Track that ApplyPinnedState is invoked (it sets frame immovable)
            ns.ApplyUnpinnedState()    -- start unpinned
            assert.is_true(ns.frame:IsMovable())

            local onClick = ns.pinBtn:GetScript("OnClick")
            onClick(ns.pinBtn)         -- click to re-pin

            -- ApplyPinnedState makes frame immovable
            assert.is_false(ns.frame:IsMovable(),
                "Frame should be immovable after pin button click when unpinned->pinned")
            assert.is_true(StatPriorityDB.pinned)
        end)

        it("pin button OnClick calls ApplyUnpinnedState when transitioning pinned->unpinned", function()
            -- Default state is pinned
            assert.is_true(StatPriorityDB.pinned)

            local onClick = ns.pinBtn:GetScript("OnClick")
            onClick(ns.pinBtn)         -- click to unpin

            assert.is_true(ns.frame:IsMovable(),
                "Frame should be movable after pin button click when pinned->unpinned")
            assert.is_false(StatPriorityDB.pinned)
        end)

        it("unpinned with no saved position gets a valid default anchor", function()
            -- When pinned=false and no position key exists, OnLoad should set
            -- a sensible default anchor (TOPRIGHT on UIParent).
            _G.StatPriorityDB    = { pinned = false }  -- no position key
            _G.StatPriorityNS    = nil
            _G.StatPriorityFrame = nil
            dofile("StatPriority/StatPriority.lua")
            local ns2 = _G.StatPriorityNS
            ...
            -- Frame must have at least one anchor point set
            local points = ns2.frame._points
            assert.is_true(#points > 0,
                "Expected at least one anchor point on frame when unpinned with no saved position")

            -- The first point must be a valid WoW anchor keyword, not nil
            local pt = points[1]
            assert.is_not_nil(pt[1],
                "Anchor point name should not be nil for default unpinned position")
        end)

        it("ApplyPinnedState followed by ApplyUnpinnedState leaves frame movable", function()
            ns.ApplyPinnedState()
            assert.is_false(ns.frame:IsMovable())
            ns.ApplyUnpinnedState()
            assert.is_true(ns.frame:IsMovable())
